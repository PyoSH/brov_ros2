...
## 6a. G4 상세 계획 — 잔차 ~44 ms 의 소재와 수단 (2026-09-03, 학습 PC)

**주의: ~44 ms 는 차감 추정이다** (실기 M3−M2 64.4 − SITL FC내부 ~20).
직접 측정이 먼저다.

**유력 기전 — FC↔RPi 직렬 링크의 burst 큐잉.** 단서 둘: (1) M2 jitter —
TIMESYNC RTT 중앙 20.6 인데 p90 38.8/최대 46.8 ms = 작은 메시지가 간헐적으로
무언가 뒤에 줄을 선다. (2) 산술 — telemetry burst(사이클당 ~400 byte)가
115200 baud(11.5 kB/s)면 **~35 ms** 직렬화 지연 = 잔차와 같은 등급.
SERVO_OUTPUT_RAW 가 burst 중간에 서면 그만큼 늦는다. SITL 은 serial0 이
UDP 라 baud 제한이 없어 이 잔차가 없다 — 차감이 맞아떨어지는 이유.

**측정 순서 (실기, 코드 불필요, ~반나절):**

| # | 확인 | 판정 |
|---|---|---|
| 1 | BlueOS 에서 FC 링크 종류·baud (UART 115200 vs USB) | UART 115200 → 주범 후보. USB 면 가설 기각 |
| 2 | 다른 소비자(Cockpit/mavlink2rest) 끄고 + SRx 축소 → M3 재측정 | 뚝 떨어지면 파이프 혼잡 확정 |
| 3 | base_node 전용 `udpin` endpoint → M2/M3 A/B | 라우터 공유 큐 몫 |
| 4 | 라우터 교체(MAVLinkServer/MAVP2P) A/B | 라우터 자체 몫 |

**수단 (결과별):** baud 범인 → `SERIALx_BAUD` 921600 (양쪽, 불안정하면
460800) — burst 35→~4 ms, 파라미터 2개로 잔차 대부분 회수. 혼잡 범인 →
주행 중 GCS/소비자 최소화 + SRx 하향. 라우터 범인 → 전용 endpoint 상시화
또는 교체. 전부 아니면 실 FC 스케줄 몫 — 이때만 온보드 이전 재검토.

**함정:** 44 ms 가 FC↔RPi 직렬에 있다면 **온보드 이전은 무효** — 그 구간을
그대로 지난다. 온보드가 먹는 것은 RPi↔랩톱(20.6 의 일부)뿐이다.

**성공 시 기대:** τ 85.3 → G1(−10) → G4(−30~35) → **~40 ms**. K_p 문턱
3.52→~5.9. 학습 주입 중심 `τ−21` ≈ 19 ms 가 되면 delayA 학습 범위(주입
40~80) **밖** — 하한 확장 재학습 사유가 된다 (DELAY_TRAINING_PLAN §5-8 의
B 저지연 붕괴 교훈과 같은 논리).

### 6a-1. 분해 도구 — 구현·대조군 검증 완료 (2026-09-03, 학습 PC)

| 도구 | 무엇 | 상태 |
|---|---|---|
| `diag_loop_delay --mode transit` | 메시지별 편도 `d = 도착(랩톱 wall) − FC stamp` → 시계 drift[ppm] + **큐잉 성분**(이동중앙값 2 s high-pass 잔차) + 토픽 간 중앙 차. `--offset` 을 주면 절대 하행 | 합성 시험 + SITL 대조군 통과 |
| `diag_link_rtt` 확장 | TIMESYNC `tc1` 로 시계 offset(FC boot − 랩톱 wall) 추정(최소-RTT 표본, ±불확도), transit 에 넘길 값을 그대로 출력 | 구현 |
| `runtime/rpi_transit_probe.py` | RPi(BlueOS) 관측점 — 같은 메시지의 (FC 도장, RPi 도착) CSV. FC→RPi vs RPi→랩톱 분리 = **온보드 이전 가치 판정** | 구현 (실기용, BlueOS 에 localhost endpoint 1개 필요) |

**대조군 결과** (도구가 참을 잰다는 증거):
- 직결(음성): 큐잉 p90 **11 ms**, "평평" — 10 ms 바닥은 우리 100 Hz 센서 폴링
  양자화(예측대로).
- mavproxy(양성): ahrs 큐잉 **~40 ms** 로 뚜렷 — mavproxy 버퍼링 검출.

**방법론적 함정 2개를 대조군이 잡았다** (실기 해석 시 유의):
1. **시계 drift** — SITL RTF 오차 ~0.2~0.35%가 큐잉으로 오독됨(가짜 p90
   200 ms). 실기 크리스털도 수십 ppm. → 선형 detrend.
2. **비선형 wobble** — SITL RTF 가 부하 따라 흔들려 선형 제거로도 부족
   (두 토픽의 잔차 프로파일이 동일 = 시계 공통 성분이라는 판별법). →
   이동중앙값 high-pass. 실기에서도 토픽 간 프로파일 비교로 시계 성분과
   큐잉을 구분할 것.

**관측 time-sync 메모**: 관측 16-D 는 요소별 최신값 조립이다 — q_e·ω 는 같은
ATTITUDE 메시지(상호 동기), v 는 별도 메시지(스냅샷 내 상호 skew ≤ 1 telemetry
주기), v_d 는 최대 1틱 늦음, 정렬·보간 없음. 학습은 완전 동기 샘플이므로
