...
> **2026-09-03 세션(delayA 배포 + 지연 분해 M1~M4·chirp) 요약은
> [session_20260903_summary.md](session_20260903_summary.md) 에 있다.**
...
## 4b. 지연 분해 — M1/M2 확정 (2026-09-03)

τ_total 80 ms 중 **링크 왕복은 20.6 ms (26 %)** 다. 계획·전체 표는
[LATENCY_DECOMPOSITION_PLAN.md](LATENCY_DECOMPOSITION_PLAN.md) §3b.

| 구간 | 값 |
|---|---|
| Fathom-X + RPi 네트워크 (ICMP) | 5.7 ms |
| 라우터 + FC 수신처리 | 15.0 ms |
| **τ_link 왕복 (M2, TIMESYNC)** | **20.6 ms** |
| 잔여 (FC 스케줄/양자화 + 액추에이터) | **59.4 ms** ← M4 가 답한다 |
| 링크 jitter (p90−p10) | 19.7 ms |

**M3/M4 로 예산이 닫혔다 (2026-09-03 01:21, `a2_yaw-20260903-012136`):**

| 구간 | 값 | 비중 |
|---|---|---|
| 링크 왕복 (M2) | 20.6 ms | 26 % |
| **FC 내부 + telemetry 생성 (M3−M2)** | **64.4 ms** | **80 %** |
| 액추에이터 + 자이로 (M4, FC 시계) | ~0 ms (< 20 ms) | ~0 % |
| M3 + M4 = 85 ms vs τ_total 85.3 ms (chirp 보정) | 차 0.3 % | **완전 정합** |

chirp(0.5→8 Hz) 재측정이 τ 를 확정했다: 부그리드 보정 **85.3 ms**, 반폭 120 → 60 ms,
**lag 0 의 상관이 −0.205**(사각파는 +0.743) — 지연이 없다면 나올 수 없는 값이라
고정 지연의 실재를 직접 보인다. 학습 주입값을 **80 → 85 ms** 로 갱신할 근거다.

**§2-2 의 "80 ms = 통신·처리 경로 자체" 를 정정한다.** 테더·라우터는 26 % 뿐이고
**ESC·추진기는 0** 이다. 80 % 가 FC 수신→override 적용→telemetry 생성이다.
따라서 `RC_SPEED` 400 Hz 도 온보드 이전도 큰 이득이 없고, 남은 수단은 telemetry
주기 상향(25→50 Hz)과 라우터 교체다 — `LATENCY_DECOMPOSITION_PLAN.md` §3c.

학습 주입에 대한 함의: **링크만으로 20 ms 폭의 jitter 가 있다.** §6 의 "1~3 스텝
랜덤"(40~120 ms) 이 이 폭과 부합하므로 고정값 주입보다 분포 주입이 맞다.
온보드 이전은 최대 20.6 ms 이득이라 우선순위가 아니다.

---

## 4c. delayA 정책 실기 A/B (2026-09-03) — **지연 DR 이 통했다**

같은 프로토콜(`runtime/a3_policy.sh`, start_heading, leg 1.5 m, rise 0.5 m,
0.25 m/s, gain 1.0, DVL off)로 두 정책을 연속 주행. delayA 는 78 s 에 상태추정이
발산했으므로(아래) **같은 55 s 창**으로 비교한다
(`A1_SECONDS=55 python3 runtime/analysis/*.py`).

bag: `a3_fixplant-20260903-005928`, `a3_delayA-20260903-010900`.

### 포화 — relay 거동이 사라졌다

| 행동 \|a\| ≥ 0.99 | fixplant | **delayA** |
|---|---|---|
| surge | 58 % | **7 %** |
| sway | 67 % | **4 %** |
| pitch | 33 % | **0 %** |
...
`fixplant` 는 55 s 에 다리 3 개를 겨우 넘겼고 yaw 는 4.7 바퀴를 **왕복**하며
제자리에서 요동했다. `delayA` 의 yaw 누적 −1357°(단조)은 **왕복마다 같은 방향
180° 선회**를 9 회 한 결과다 — 즉 미션을 실제로 수행했다. ω_yaw RMS 가 1.04 로
높은 것은 그 선회 때문이지 진동이 아니다.

### 2 Hz 진동대 (1.8~2.6 Hz 절대 RMS)

| 축 | 명령 fixplant → delayA | 응답 fixplant → delayA |
|---|---|---|
| surge | 15.6 → **6.7 N** (×0.43) | 0.363 → 0.281 m/s² (×0.77) |
| sway | 17.4 → **10.8 N** (×0.62) | 0.826 → 0.641 (×0.78) |
| heave | 11.9 → **7.9 N** (×0.66) | 0.418 → 0.297 (×0.71) |
| roll | 11.0 → **4.0 N·m** (×0.36) | 6.85 → 4.79 rad/s² (×0.70) |
| pitch | 5.0 → **1.3 N·m** (×0.26) | 4.50 → 2.17 (×0.48) |
| yaw | 7.0 → **1.7 N·m** (×0.24) | 3.89 → 1.66 (×0.43) |

### 판정

`deadtime_result_to_training.md` §6 의 수용 기준 3 개 중 **2 개 통과**:
포화 < 10 % (surge 7, sway 4, roll/pitch 0 — yaw 18 만 미달),
sway 2 Hz < 5 N 은 **미달(10.8 N)**, surge ≥ 0.2 m/s 는 **근접(0.172)**.

기전 판정은 명확하다. A1 에서 이득만 낮췄을 때는 진폭만 줄고 포화가 남았는데
(58→62 %), 지연을 **학습에 넣자 포화 자체가 사라졌다**(58→7 %). 정책이 스스로
고주파 이득을 낮춘 것이고, 이것이 relay limit cycle 의 근본 대응이 맞다.

남은 것은 **sway·yaw** 다. 두 축만 여전히 2 Hz 성분과 포화(18 %)가 있다 —
sway–roll 커플링이 지연 DR 만으로는 완전히 안 잡혔다.

> **정정 (실험 5, `heading_mode:=straight`).** 그 두 축은 지연 DR 의 한계가 아니라
> **미션 기하가 만든 것**이었다. 180° 선회를 없애자 sway ×0.25, yaw ×0.15, roll
> ×0.13 으로 무너지고 포화도 0 % 가 됐다. **surge 만 ×1.00 으로 남는다**(6.7 N) —
> 그것이 dead-time limit cycle 의 진짜 잔여다. `session_20260903_summary.md` 실험 5.
...
| 80~90 s | **2.1~2.3 m/s** | 0.10~0.24 rad/s | −0.17~+0.06 m |

자이로와 Bar30 은 정상인데 EKF 속도만 2 m/s 대로 튀었다. 4 m 수조에서 9.6 m
이동·최대 3.1 m/s 는 물리적으로 불가능하다 — **EKF 상태추정 발산**이다.
`/brov/state.valid` 는 내내 true 였다(0 % invalid) — **이 게이트는 발산을 못 잡는다.**
DVL 기록이 꺼져 있어 A50 상태를 볼 수 없었다. 같은 방향 180° 선회를 9 회 반복한
뒤라 DVL bottom lock 상실이 1 순위 후보다. 다음 주행에서는 `check_ekf.sh` 를
주행 직후 돌리고, `/brov/state` 에 EKF flags 를 실어 bag 에 남길 것.
...
- **v5 는 순수 w_a=0.3 대조군이 아니다** (`reward_profile: deploy_v2`). "w_a 를 낮춰서
  ...
  Jacobian 을 같은 관측점에서.
  > **수행됨 (2026-09-03, 학습 PC) — 가설 확정.** `fixplant_wa03_s42`(paper_ref_v1,
  > 같은 plant·seed)를 export 해 같은 관측점(base_d0 순항 417표본 + 공통 anchor)에서
  > 재니 K_p = **1.34/1.65/1.51** (surge/sway/heave). w_a=0.017 은 같은 지점에서
  > 7.23/6.46/6.14 — **w_a 0.3→0.017 이 병진 이득을 4~5배 올렸다.** 함의:
  > (1) 논문 원본 w_a=0.3 은 80 ms 문턱(3.52)의 38~47% 아래라 **지연에 안정**하지만
  > 추종 16.4% 로 못 쓴다. (2) 추종을 살린 보상 수정이 곧 지연 불안정의 원인이다 —
  > 진동과 w_a 수정은 인과로 연결돼 있다. (3) delayA 가 이 딜레마의 해다
  > (K_p 2.6 < 3.52 **이면서** 추종 100%). (4) 논문 실기가 안 떨었다면(질문 목록
  > §7) 낮은 이득 덕일 가능성 — "heave 응답 느림" 보고와 정합.
  > 산출물: `deploy/exported_wa03/`, 실행은 `delay_gate/jacobian_ident.py`.
- **deadband 추력**: T200 역변환 0.25 N → PWM 0.000, 0.5 N → 0.091 → 추진기당 ~0.45 N
  아래는 출력 0. 순항 0.25 m/s 는 추진기당 4.3 N(PWM 0.22)으로 밖.
- **추진기 한계 불일치**: ~~확인할 것~~ → **해소됨 (2026-09-02, 학습 PC).** 두 숫자
  모두 실재하고 의미가 다르다: −49.4/+65.9 = `force_limits_n` 전 전압 포락선(보상
  정규화 전용 — 구 로그가 이걸 찍어 오도, 로그 수정됨), **−36.7/+47.2 @14.8 V =
  `clamp_thrust` 실제 클램프**(전압 의존: 12.6 V −30.3/+38.8, 16.8 V −41.9/+54.5).
  ±51.5/64.1 을 단언하던 시험은 실동작 기준으로 갱신되어 통과한다.
...
| 넣을 것 | 값 | 근거 |
|---|---|---|
| 행동 지연 | **주입 중심 ~64 ms** (= τ_total 85.3 − 시뮬 액추에이터 등가 21), 랜덤 폭은 링크 jitter 반영 예: 40~90 ms | §2-2, §4b + **§6-보강 ① 이중 계상 보정** |
...
> **정정 (2026-09-03, 학습 PC):** 세션 요약의 "85 ms 중심, 40~120" 은 §6-보강 ①의
> 이중 계상 보정을 놓친 값이다. 학습 환경의 von Benzon 3차 액추에이터가 2 Hz 에서
> 이미 **등가지연 21 ms** 를 공급하므로(실기 M4 ≈ 0~20 ms 와 정합 — 모델과 실기
> 액추에이터가 같은 등급), 시뮬 총지연 = 주입값 + 21 이다. 85 를 주입하면 총 106 ms
> 로 실기 85.3 을 25 % 초과한다 — IsaacLab 단계 0 이 정확히 이것을 실측했다
> (주입 60 → 2.28 Hz = 실기 재현, 주입 80 → 1.92 Hz = 과잉). 올바른 중심은
> **85.3 − 21 ≈ 64 ms** 다. 방증: delayA(주입 40~80, 시뮬 총 61~101)가 실기 총
> 85.3 을 정확히 품었고, 실기에서 통했다. G1~G3 해소로 실기 τ 가 줄면 이 값도
> 같이 내린다 (`τ_주입 = τ_total − 21`).
