...
무관한 순수 통신 계층이다(반환값은 다른 brov_base 모듈과 맞추기 위해 torch
텐서로 감싼다).

SERVOn_FUNCTION 자동 백업/복원
-------------------------------
`enable_passthrough()`/`disable_passthrough()`가 8개 스러스터 채널의
`SERVOn_FUNCTION`을 RCPassThru(=1)로 바꾸고 원복한다. `disable_passthrough()`를
호출하지 않고 끝내면 RCPassThru가 걸린 채로 남아
GUIDED/AUTO 등 ArduSub 내장 제어기가 겉으로는 정상(arm 성공, 에러 없음)인데
스러스터에는 반영되지 않을 수 있다. 이것이 `close()`가 항상 parameter를 read-back
검증하며 원복하는 이유다.
...
class RealRobotInterface:
    ...
    # ------------------------------------------------------------------
    # SERVOn_FUNCTION 백업/RCPassThru 전환. THRUSTER_RC_CHANNELS와 무관하게
    # 항상 SERVO1~8을 다룬다
