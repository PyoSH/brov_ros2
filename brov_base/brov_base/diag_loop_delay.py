...
_M_EFF = {"surge": 14.635 + 6.36, "sway": 14.635 + 7.12, "heave": 14.635 + 13.5}
_WRENCH_SCALE = {"surge": 85.0, "sway": 85.0, "heave": 120.0}
_AXIS_INDEX = {"surge": 0, "sway": 1, "heave": 2}
...
def analyse(wr_t, wr_f, st_t, st_v, act_t, act_v, axis: str, m_eff: float,
    ...
    # 위상 예산과 예측 진동 주파수
    print(f"\n위상 예산  (m_eff = {m_eff:.1f} kg, 제어주기 {control_dt*1000:.0f} ms)")
    phi_c = 4.0                      # 정책 실측 -1~-5도. 보수적으로 4도.
    f_pred = (90.0 - phi_c) / (360.0 * (tau + control_dt / 2))
    print(f"  -180도 교차 예측 주파수 = {f_pred:5.2f} Hz")
    if math.isfinite(ff) and ff > 0.2:
        print(f"  실측 지배 주파수        = {ff:5.2f} Hz"
              f"   ({'일치' if abs(ff - f_pred) < 0.5 * f_pred else '불일치 — 다른 위상원 의심'})")
    w = 2 * math.pi * f_pred
    kp_thresh = m_eff * w / scale
    print(f"  이 주파수에서 |L|=1 이 되는 이득 문턱 K_p = {kp_thresh:5.2f}")
    print(f"    (학습된 정책 실측 K_p ~ 4.5 -- 넘으면 진동 조건 성립)")
...
def main() -> None:
    ...
    ap.add_argument("--m-eff", type=float, default=None,
                    help="유효질량 [kg]. 미지정 시 축별 기본값(강체+added mass)")
